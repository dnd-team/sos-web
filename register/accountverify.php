...
<head>
    ...
    <title>SOS - Account bestätigen</title>
...
<body id="verify">
    <div class="demo-layout mdl-layout mdl-js-layout mdl-color--grey-100">
        <header class="demo-header mdl-layout__header mdl-layout__header--scroll mdl-color--grey-100 mdl-color-text--grey-800">
                <div class="mdl-layout__header-row">
                    <span class="mdl-layout-title">SOS - Account best&auml;tigen</span>
    ...
    <script type="text/javascript">
        $(function() {
            ...
            var verificationCode = getURLParameter("v");
            var error =  '<h3>Fehler beim Verifizieren!</h3><br><p>Bitte versuche es erneut oder wende Dich an unseren Support: support@schoolos.de</p><br><a href="../login" target="_self" class="mdl-button">Zum Login</a>';
            if (email != "" && email != " " && verificationCode != "" && verificationCode != " " && email != null && verificationCode != null) {
                verifyAccount(verificationCode, email);
            } else {
            ...
            function verifyAccount(verificationCode, email) {
                $.ajax({
                    url: apiURL + "/verify",
                    data: {
                    ...
                    success: function(data) {
                        console.log(data);
                        if (data["message"] == "Error. User could not be verified.") {

                            $('#main-card').html(error).promise().done(function() {
                                componentHandler.upgradeDom();
                            });
                        } else if (data["message"] == "Account successfully verified.") {
                            var success =  '<h3>Account erfolgreich verifiziert!</h3><br><a href="../login" target="_self" class="mdl-button">Zum Login</a>';
